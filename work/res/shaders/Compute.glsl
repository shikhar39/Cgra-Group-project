...
void main() {
    vec4 value = vec4(0.0, 0.01, 0.0, 0.0);
    float speed = 100;
    ...
    if(particles[gl_GlobalInvocationID.x].position.y < 00.f) {
        particles[gl_GlobalInvocationID.x].velocity.y = - particles[gl_GlobalInvocationID.x].velocity.y;
    }
