...
void main() {
    ...
    if( idx.x == 0 && Pulse) {
        gridData[idx.y * gridCountX + idx.x ].height = gridData[idx.y * gridCountX + idx.x ].height + 1.5;
    }
    uvec2 n1, n2, n3, n4;
    if (idx.x != 0) { 
        n1 = uvec2(idx.x - 1, idx.y);
        if (gridData[n1.y * gridCountX + n1.x ].height < gridData[n1.y * gridCountX + n1.x ].terrainHeight){
            n1 = uvec2(idx.x, idx.y);
        }
    } else { 
        n1 = uvec2(idx.x, idx.y);
    }
    if (idx.y != gridCountY - 1) {
        n2 = uvec2(idx.x , idx.y + 1); 
        if (gridData[n2.y * gridCountX + n2.x ].height < gridData[n2.y * gridCountX + n2.x ].terrainHeight){
            n2 = uvec2(idx.x, idx.y);
        }
    } else { 
        n2 = uvec2(idx.x, idx.y); 
    }
    if (idx.x != gridCountX - 1) { 
        n3 = uvec2(idx.x + 1, idx.y); 
        if (gridData[n3.y * gridCountX + n3.x ].height < gridData[n3.y * gridCountX + n3.x ].terrainHeight){
            n3 = uvec2(idx.x, idx.y);
        }
    } else { 
        n3 = uvec2(idx.x, idx.y);  
    }
    if (idx.y != 0) { 
        n4 = uvec2(idx.x, idx.y - 1); 
        if (gridData[n4.y * gridCountX + n4.x ].height < gridData[n4.y * gridCountX + n4.x ].terrainHeight){
            n4 = uvec2(idx.x, idx.y);
        }
    } else { 
        n4 = uvec2(idx.x, idx.y); 
    }
